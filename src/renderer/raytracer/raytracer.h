...
namespace cg::renderer
{
	...
	public:
		...
		std::function<payload(const ray& ray)>
				miss_shader = nullptr;
		std::function<payload(const ray& ray,
							  payload& payload,
	...
	{
		float frame_wight = 1.f / static_cast<float>(accumulation_num);
...
#pragma omp parallel for
				for (int y = 0; y < height; ++y) {
					...
					render_target->item(x, y) = RT::from_color3(history_pixel);
				}
			}
		}
	}
			...
			const triangle<VB>& triangle, const ray& ray) const
	{
		...
		float3 pvec = linalg::cross(ray.direction, triangle.ca);
		float det = linalg::dot(triangle.ba, pvec);
		...
		float u = linalg::dot(tvec, pvec) * inv_det;

		if (u < 0.f || u > 1.f) {
			return payload;
		}

		float3 qvec = linalg::cross(tvec, triangle.ba);
		float v = linalg::dot(ray.direction, qvec) * inv_det;

		if (v < 0.f || u + v > 1.f) {
			return payload;
		}

		payload.t = linalg::dot(triangle.ca, qvec) * inv_det;
